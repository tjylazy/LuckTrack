(function($) {
	...
	$.fn.limit = function(o,obj){
			if(!$('.jMax-text',obj).length){
				...
				_jMaxtext.html('<span style="font">'+o.limit+'</span>');		
				...
				var _jMaxinput		= $(document.createElement('input')).attr('type','submit').attr('disabled','true').addClass('disabled').val('发布');		
				...
				_jMaxinput.click(function(e){
					show(jQuery.trim(_jMaxtextarea.val()));
				})
				if(o.showtext)
...
function show(a)
{
	document.getElementById('fabuyixiaba').value=a;
